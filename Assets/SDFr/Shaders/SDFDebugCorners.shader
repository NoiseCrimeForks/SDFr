	...
	SubShader
    {
        ...
        { 
            ...
            OutputPS Raymarch( float3 roWS, float3 rdWS, float3 reWS )
            {
                ...
                if ( intersection.x < intersection.y && intersection.x < 1 )
                {
                    ...
                    while( dist < distanceInVolume && steps < MAX_STEPS )
                    {
                        ...
                        //the ray position at current step normalize into volume texture space 
                        float3 rayPosNormalised = rayPosLS / (_SDFVolumeExtents.xyz*2);
						float d = DistanceFunction(rayPosNormalised);
                        
                        UNITY_BRANCH
                        if ( d < eps )
                        {
						//	float3 normalLS = GenerateNormalsFast(normalDelta, d, rayPosLS);
							float3 normalLS = GenerateNormals(normalDelta * 0.5, d, rayPosNormalised);
            ...
            {            
                ...
                float3 re = ro + rd * _ProjectionParams.z + _ProjectionParams.y;
